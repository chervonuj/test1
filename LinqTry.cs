...
namespace ConsoleApp1
{
    class LinqTry
    {
        ...
        {
            int[] numbers = { 5, 4, 1, 7, 2, 0 };

            int[] numbers2 = {3, 9, 8, 6, 7, 2, 0 };
            ...
            var firstn = numbers.Concat(numbers2).Distinct().OrderBy(x=>x);
            
            foreach (var x in firstn)
            {
                Console.WriteLine(x);
            }
        }

        public static void LinqProd()
        {
            ...
            var list = products.Where(x => x.UnitPrice > 10 && x.ProductName.Length<10).OrderBy(x =>x.ProductName.Length).GroupBy(x=>x.Category).Select(x=> new { Product = x, Category = x.Key});

              var list2 =
                  from p in products
                  join c in categories on p.Category equals c
                  where p.ProductName.Length < 15
            ...
            try
            {
                res = products.First(x => x.ProductName.ToLower()[0] == 's').ToString();
            }
            catch(Exception ex)
            {
                res = ex.Message;
            ...
            var listRes = products.Where(x => x.ProductName.ToLower()[0] == 's');
    ...
    public class Product
    {
        ...
        {
            return ProductName+'\t'+Category+ '\t' +UnitPrice+ '\t' + UnitsInStock;
        }
    }
